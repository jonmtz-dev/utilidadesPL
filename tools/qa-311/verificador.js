...
window.VERIFICADOR_QA_311 = function (DATOS, evidencia) {
    'use strict';
    ...
    /* El espacio tras el marcador es OPCIONAL: en una celda de tabla Moodle
       pega el número al texto —«1.Bañarse»— y exigiendo el espacio el cotejo no
       reconocía la celda, la daba por faltante y además reportaba «1.Bañarse»
       como texto de más. Dos errores por el mismo espacio.

       Sin espacio se pide que lo siguiente sea una LETRA: así «1.5 litros» no
       pierde su «1.» y se queda en «5 litros». */
    function sinPrefijoDeLista(s) {
        return String(s || '')
            .replace(/^\s*(?:[•·-]|\d+[.)]|[a-z][.)])\s+/i, '')
            .replace(/^\s*(?:[•·-]|\d+[.)]|[a-z][.)])(?=[a-záéíóúñü])/i, '');
    }

    /* Una celda cuyo texto es SOLO el número de la lista («2.», «3.») es una
       fila de plantilla que el estudiante va a llenar: en el Word esa celda
       está vacía —el número lo pone la numeración— así que no hay nada que
       cotejar y reportarla como «texto de más» es ruido en toda tabla con
       renglones en blanco. */
    function esSoloMarcador(s) {
        return /^\s*(?:[•·-]|\d+[.)]|[a-z][.)])\s*$/i.test(String(s || ''));
    }
    ...
    /* La raíz del contenido en 3.11. El montaje envuelve todo en
       `.prepa-M{n}-body`, así que se busca primero la del módulo del guion: si
       la página fuera de otro módulo, no la encuentra y el QA avisa en vez de
       cotejar contra un contenido que no es. Los tres respaldos son para las
       páginas viejas que se montaron sin esa clase. */
    var raizActividad = (DATOS.modulo && document.querySelector('.prepa-M' + DATOS.modulo + '-body'))
        || document.querySelector('[class*="prepa-M"][class$="-body"]')
    ...
    function revisarActividad(esperado) {
        ...
        nodos.filter(function (x) { return !x.usado; }).forEach(function (x) {
            ...
            if (!parteDeAlgo && !esSoloMarcador(x.t)) anotar('aviso', 'Sobra', 'Texto que no está en el guion', '', x.t, x.n);
        });
        ...
        /* Tablas: solo el número de columnas.

           Aquí NO se revisa el `data-label`. Esa comprobación viene del QA de
           5.1, donde la hoja del aula convierte cada <tr> en una tarjeta en el
           celular y sin `data-label` la tarjeta sale sin encabezados. **Moodle
           3.11 usa otros estilos**: sus tablas no llevan
           `tabla-responsive-cards` ni nada que dependa del atributo, así que
           pedirlo aquí marcaba en naranja «25 celdas sin él» en una tabla
        ...
        (esperado.tablas || []).forEach(function (tabla, i) {
            ...
            /* Las columnas son las de la CUADRÍCULA, no los <th> que haya en el
               <thead>. Contando los <th> a secas, una tabla con renglón de
               título combinado a lo ancho («Monitoreo de actividades…») daba
               1 + 5 = 6 columnas contra las 5 del Word, y reportaba un error
               inventado sobre un montaje correcto. Se toma el máximo de la suma
               de colspan por fila, que es el ancho real de la tabla. */
            var columnas = [].slice.call(t.rows).reduce(function (max, fila) {
                var suma = [].slice.call(fila.cells).reduce(function (n, c) { return n + (c.colSpan || 1); }, 0);
                return suma > max ? suma : max;
            }, 0);
            if (columnas !== tabla.encabezados.length) {
                anotar('error', 'Tablas', 'La tabla ' + (i + 1) + ' tiene otro número de columnas',
                    tabla.encabezados.length + ' columnas', columnas + ' columnas', t);
            }
        });
    ...
    /**
     * La rúbrica, en la pantalla «Definir rúbrica».
     *
     * La lectura del DOM y las reglas son las de Adaptador de Rúbricas, que es
     * quien la escribe: se lee el `textarea` de cada criterio y el de cada
     * nivel, más el `input` del puntaje. Tienen que ser las mismas reglas —y el
     * mismo lector del Word, `assets/qa-rubrica.js`— porque si el QA construyera
     * su propia versión del texto esperado, aprobaría algo mal escrito.
     *
     * Se usa DESPUÉS de guardar: al reabrir la pantalla los textareas traen lo
     * que quedó en la base de datos, no lo que se acaba de teclear.
     *
     * Tres grados, no un sí/no: `igual`, `solo espacios` (Word y Moodle nunca
     * coinciden en saltos y espacios finales, y reportarlo como error sería
     * ruido en todas las rúbricas) y `distinto`.
     */
    function revisarRubrica(esperado) {
        ...
        /* Primero por nombre exacto y luego por contenido: al montar se recorta
           o se alarga el nombre del criterio («Cognitivo» / «Criterio
           cognitivo») y buscarlo solo exacto lo daba por faltante. */
        function buscarCriterio(nombre) {
            var n = firma(nombre), i;
            for (i = 0; i < existentes.length; i++) {
                if (!existentes[i].usado && firma(existentes[i].nombre) === n) {
                    existentes[i].usado = true; return existentes[i];
                }
            }
            for (i = 0; i < existentes.length; i++) {
                if (existentes[i].usado) continue;
                var en = firma(existentes[i].nombre);
        ...
        var criterios = (esperado.criterios || []).filter(function (c) { return c.incluir; });
        var revisados = 0, correctos = 0;

        criterios.forEach(function (crit) {
            var ex = buscarCriterio(crit.nombre);
            if (!ex) {
                anotar('error', 'Rúbrica', 'Falta el criterio «' + crit.nombre + '»',
                    crit.nombre, '', null);
                return;
            }

            crit.celdas.forEach(function (celda, i) {
                var real = ex.niveles[i];
                var etiqueta = crit.nombre + ' · nivel ' + (i + 1)
                    + ((esperado.niveles && esperado.niveles[i]) ? ' (' + esperado.niveles[i] + ')' : '');
                if (!real) {
                    anotar('error', 'Rúbrica', 'Falta el nivel ' + (i + 1) + ' de «' + crit.nombre + '»',
                        celda.texto, '', null);
                    return;
                }
                revisados++;

                if (real.texto === celda.texto) {
                    correctos++;
                } else if (limpiar(real.texto) === limpiar(celda.texto)) {
                    // Mismo contenido, distinto espaciado: aviso, no error.
                    anotar('aviso', 'Rúbrica', etiqueta + ': solo cambia el espaciado',
                        celda.texto, real.texto, real.td);
                } else {
                    anotar('error', 'Rúbrica', etiqueta + ': el texto no coincide',
                        celda.texto, real.texto, real.td);
                }

                /* Los puntos se comparan como NÚMERO: «40» y «40.0» son lo
                   mismo, y compararlos como texto daba errores inventados. */
                var pEsp = parseFloat(String(celda.puntos).replace(',', '.'));
                var pReal = parseFloat(String(real.puntos).replace(',', '.'));
                if (isNaN(pEsp) || isNaN(pReal) || Math.abs(pEsp - pReal) >= 0.0001) {
                    anotar('error', 'Rúbrica', etiqueta + ': el puntaje no coincide',
                        String(celda.puntos), String(real.puntos), real.td);
                }
            });
        });

        /* Un criterio que está en Moodle y no en el Word: sobra, o se le cambió
           tanto el nombre que no se reconoció. Las dos cosas hay que verlas. */
        existentes.filter(function (e) { return !e.usado; }).forEach(function (e) {
            anotar('aviso', 'Rúbrica', 'En Moodle hay un criterio que no está en el Word',
                '', e.nombre, e.tr);
        });

        return { revisados: revisados, correctos: correctos };
    }
    ...
    function generarEvidencia(modo, resumen, estado, color) {
        ...
        if (esDeRubrica && DATOS.rubrica) {
            ficha.push(['Criterios', String((DATOS.rubrica.criterios || []).filter(function (c) { return c.incluir; }).length) + ' cotejados']);
    ...
    function pintar(modo, resumen) {
        [].slice.call(document.querySelectorAll('.qa311-marca')).forEach(function (n) {
            n.style.outline = '';
            n.classList.remove('qa311-marca');
        });
        var viejo = document.getElementById('qa311-panel');
        ...
        hallazgos.forEach(function (h) {
            ...
            h.nodo.classList.add('qa311-marca');
        });
        ...
        var html = '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px">'
            + '<strong style="font-size:15px">QA ' + esc(modo) + ' · Moodle 3.11</strong>'
            + '<button id="qa311-cerrar" style="border:0;background:#eee;border-radius:6px;padding:4px 10px;cursor:pointer">Cerrar</button></div>'
            ...
            + '<button id="qa311-evidencia" style="width:100%;border:1px solid ' + color + ';background:#fff;color:'
        ...
        document.getElementById('qa311-evidencia').onclick = function () {
            generarEvidencia(modo, resumen, estado, color);
        };
        document.getElementById('qa311-cerrar').onclick = function () {
            [].slice.call(document.querySelectorAll('.qa311-marca')).forEach(function (n) {
                n.style.outline = '';
                n.classList.remove('qa311-marca');
            });
        ...
        var primero = document.querySelector('.qa311-marca');
    ...
    if (esRubrica) {
        ...
        if (!r) return;
        return pintar('de rúbrica', r.revisados + ' celdas del Word cotejadas · ' + r.correctos + ' idénticas');
    }
