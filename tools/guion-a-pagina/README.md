...
## El color de los botones (y por qué el gris salía claro)

Botón, Ventana emergente y cada tarjeta traen **Color del botón**: *Del tema*
(`btn-primary`) y *Gris* (`btn-secondary`). Antes no había campo: todos salían
grises.

El markup es el de una página ya montada, y se copia entero:

```html
<button class="btn btn-primary btn-lg rounded-4 border border-4 border-primary-10"
        style="margin-bottom: 5px;" type="button"
        data-bs-toggle="modal" data-bs-target="#modal1">Anillo 1- Factor humano</button>
```

Tres detalles que **no se deducen** y que estaban mal:

- El borde es **`border-primary-10` en los dos colores**. El gris llevaba
  `border-secondary-10`, que es lo coherente y no es lo que hay en la página.
  (De paso: `.border-primary-10` **no existe** en la hoja de Moodle —hay
  `-20`, `-30`, `-40`, `-50`—, así que en realidad deja el borde gris claro de
  Bootstrap. Aun así se copia tal cual: es lo que hay en la página que se ve
  bien.)
...
Ver la sección siguiente. En corto: el gris fuerte lo da
`.ms-convertido .btn-secondary`, y la salida no llevaba esa clase.

### Tarjetas sin imagen = grupo de botones

La marca `<Crear un grupo de N botones>` del guion entra como bloque **Tarjetas**,
y salía igual que las "culturas": cada botón dentro de su `.card`. En Moodle eso
se ve como una **banda gris con los botones flotando dentro**, que no es ningún
montaje. El grupo de verdad es una columna centrada y los botones sueltos:

```html
<div class="col-12 col-lg-12 mx-auto text-center"> … botones … </div>
```

El campo **Se ve como** (*Automático · Tarjetas · Solo botones*) lo decide, y en
*Automático* —el valor de fábrica— manda la imagen: si ninguna tarjeta trae una,
sale el grupo de botones. Los botones nuevos nacen **alternando** tema y gris,
como en la página publicada.

De paso se va la **banda gris** que salía alrededor de esos botones en Moodle y
...
```html
<div class="container-fluid mainPlantilla23 M01 ms-convertido" style="max-width: 100% !important;">
```

`ms-convertido` **no** significa "esto vino de un micrositio". Es el interruptor
con el que la hoja del aula pinta este tipo de contenido, y las páginas de
montaje que sirven de referencia lo llevan. Sin él, la misma clase se ve
distinta. Lo que cambia (todo sale de `conjunto_unificado.scss`):

| Regla | Sin la clase | Con la clase |
|---|---|---|
| `.btn-secondary` | el gris claro de Moodle | `#6c757d` con letra blanca, y su hover y active |
| `.accordion-button:hover` | `primary-30` con letra `primary-50` | `primary-40` con **letra blanca** |
| `.table-responsive .rounded-top p` | normal | negritas y centrada |
| `.card-deck` | flex (vuelve fila el pie de figura) | bloque |
| `.card.img-contenedor` | sin fondo | blanco con borde |
| `.bloque` | sin relleno | `8px` arriba y abajo |
| `a` | color del tema | **azul y subrayado** |
| `.flecha_btn` | nada | la flechita del botón |

Ese fue el reporte que abrió todo esto: el gris de los botones salía pálido. No
era el markup del botón —ese ya estaba bien—, era que a la página le faltaba la
clase.

**El renglón de `a` viene con cola y se aceptó a sabiendas.** La hoja manda
`color: blue !important; text-decoration-line: underline !important`, y un
`!important` de hoja le gana a cualquier `style=` en línea: desde la herramienta
**no se puede revertir**. Salen azules y subrayados los enlaces del texto, el
Botón cuando se ve como botón, y la palabra que abre una ventana (su
`text-decoration-none` pierde por especificidad). Así se ven las páginas del
aula; si algún día no debe ser así, el arreglo va en la hoja, no aquí.

> `CSS_VISTA_PREVIA` copia estas reglas al final, con la misma cascada. Es feo
> ver el enlace azul en la previa y es justo el punto: la previa que pinta
> bonito lo que Moodle pinta azul está mintiendo.

Al leer una página de vuelta, `ms-convertido` cuenta como clase **propia** y no
como "clase extra" (`importar-html.js`); si no, se duplicaría en cada ida y
vuelta.
...
| Tarjetas | `.card-group > .card` con botón `btn-<color> btn-sm rounded-4 border border-4 border-primary-10` |
| Grupo de botones | `.col-12.col-lg-12.mx-auto` con botones `btn-<color> btn-lg rounded-4 border border-4 border-primary-10` y `margin-bottom: 5px` en línea |
