...
/** Copia el look computado de `src` (render fiel) a estilos inline de `dst`. */
function congelarElemento(src, dst) {
    const cs = getComputedStyle(src);
    const cls = ' ' + (typeof src.className === 'string' ? src.className : '') + ' ';
    const esBtn = /\sbtn(-\S+)?\s/.test(cls) || /\sbtn\s/.test(cls);
    const esAcc = /\saccordion-button\s/.test(cls);
    const esTh = src.tagName === 'TH';

    // Fondo. El TH y el botón/acordeón heredan el color visible de un ancestro
    // (p. ej. el thead coloreado): tomamos el fondo EFECTIVO, no el propio.
    if (RE_BG.test(cls) || esBtn || esAcc || esTh) {
        const bg = (esTh || esAcc || esBtn) ? fondoEfectivo(src) : cs.backgroundColor;
        if (bg && !esTransparente(bg)) dst.style.setProperty('background-color', bg, 'important');
    }
    // Texto.
    if (RE_TX.test(cls) || esBtn || esAcc || esTh) {
...
async function blindar(doc, cssMicro) {
    ...
    try {
        ...
        for (let i = 0; i < src.length; i++) congelarElemento(src[i], dst[i]);
    } finally {
        iframe.remove();
    }
}
